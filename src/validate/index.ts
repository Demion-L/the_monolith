...
function readMonolithRoot(targetDir: string): string {
  const configPath = join(targetDir, 'monolith.config.json');
  if (!existsSync(configPath)) return '.monolith';
  try {
...
async function checkConfiguration(targetDir: string): Promise<CheckResult> {
  const configPath = join(targetDir, 'monolith.config.json');
...
function checkInstance(targetDir: string, monolithRoot: string): CheckResult {
...
function hasUnreplacedInitVars(content: string): boolean {
  let match: RegExpExecArray | null;
  INIT_PLACEHOLDER_RE.lastIndex = 0;
  while ((match = INIT_PLACEHOLDER_RE.exec(content)) !== null) {
    if (INIT_VARS.has(match[1])) return true;
  }
  return false;
}

function checkTemplates(targetDir: string, monolithRoot: string): CheckResult {
...
export async function runValidate(options: ValidateOptions): Promise<void> {
  ...
  const results: CheckResult[] = [configResult, instanceResult, templateResult];
  const passed = results.every((r) => r.passed);

  if (quiet) {
    if (!passed) {
      for (const r of results) {
        if (!r.passed) console.log(formatRow(r));
      }
    }
  } else {
    console.log('');
