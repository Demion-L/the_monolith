 ...
 */
export function selectCanonicalLabel(variants: string[]): string {
  if (variants.length === 0) return '';
  if (variants.length === 1) return variants[0];

  function titleCaseScore(label: string): number {
  ...
  return [...variants].sort((a, b) => {
    ...
    if (aUpper !== bUpper) return aUpper - bUpper; // fewer raw uppercase = more mixed-case = more readable
