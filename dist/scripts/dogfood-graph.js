...
// Build graph
// ---------------------------------------------------------------------------
const graph = buildGraph(compiled.map(e => e.result));
// ---------------------------------------------------------------------------
// Analysis helpers
// ---------------------------------------------------------------------------
...
${relTypeRows.length > 0
    ? mdTable(['Relationship type', 'Edge count'], relTypeRows.map(([type, count]) => [type, String(count)]))
...
${topConnected.length > 0
    ? mdTable(['Node', 'Canonical label', 'Degree', 'Sources', 'Synthesized?'], topConnected.map(n => [
        n.id,
        n.canonicalLabel,
...
${mergedNodes.length > 0
    ? mdTable(['Canonical label', 'Aliases'], mergedNodes.slice(0, 20).map(n => [n.canonicalLabel, n.aliases.join(', ')])) + (mergedNodes.length > 20 ? `\n\n… and ${mergedNodes.length - 20} more merged nodes.` : '')
...
${relativelyRichFm.length > 0
    ? mdTable(['File', 'Relationship candidates'], relativelyRichFm.map(e => [e.relPath, String(e.relCount)]))
...
${goodEdges.length > 0
    ? mdTable(['From', 'Type', 'To', 'Source artifact'], goodEdges.map(e => [
...
${noisyEdges.length > 0
    ? mdTable(['From', 'Type', 'To (unresolved)', 'Source artifact'], noisyEdges.map(e => [
        nodeLabel(e.fromNodeId),
        e.relationshipType,
        nodeLabel(e.toNodeId),
        e.sourceArtifactId.split('--').slice(-1)[0] ?? e.sourceArtifactId,
...
${mdTable(['Problem', 'Recommended fix'], [
    [
