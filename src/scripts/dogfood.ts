...
import type { Artifact, WebwayAnalysisResult } from '../webway/types.js';
...
function findMdFiles(dir: string): string[] {
  ...
  for (const entry of entries) {
    if (entry.startsWith('.git') && entry !== '.monolith') continue;
...
try { statSync(readmePath); targets.push(readmePath); } catch { /* absent */ }
...
interface FileResult {
  ...
  result: WebwayAnalysisResult;
}

const analyzed: FileResult[] = [];
...
const conceptFileCount = new Map<string, number>();
...
for (const { relPath, result } of analyzed) {
  ...
  for (const c of result.concepts) {
    if (!seen.has(c.label)) {
      ...
      if (!conceptFiles.has(c.label)) conceptFiles.set(c.label, []);
...
for (const { result } of analyzed) {
  ...
  if (!regionCount.has(r.id)) regionCount.set(r.id, { name: r.name, count: 0 });
...
const conceptIdToLabel = new Map<string, string>();
for (const { result } of analyzed) {
  for (const c of result.concepts) conceptIdToLabel.set(c.id, c.label);
}
...
const noisyFiles: { file: string; concepts: string[]; noiseCount: number }[] = [];
const signalFiles: { file: string; concepts: string[]; region: string; confidence: number }[] = [];
...
${mdTable(
  ['Region', 'Files', 'Share'],
  regionDist.map(r => [r.name, String(r.count), pct(r.count, totalAnalyzed)])
)}
...
${mdTable(
  ...
  topKnowledgeConcepts.map(c => [c.label, String(c.count), pct(c.count, totalAnalyzed)])
)}
...
${mdTable(
  ['Concept', 'Files', 'Noise?'],
  topConcepts.slice(0, 20).map(c => [c.label, String(c.count), STRUCTURAL_NOISE.has(c.label) ? 'YES' : '-'])
)}
...
${mdTable(
  ...
  repeatedConcepts.map(c => [c.label, String(c.count), STRUCTURAL_NOISE.has(c.label) ? 'noise' : 'signal'])
)}
...
${mdTable(
  ...
  strongestLinks.map(l => [l.from, l.relationship, l.to, l.strength.toFixed(2)])
)}
...
${skipped.length > 0
  ? mdTable(
      ['File', 'Reason'],
      skipped.map(s => [s.file.replace('.monolith/', ''), s.reason])
    )
  : '_None._'
}
